...
class CRM_MembershipExtras_Job_OfflineAutoRenewal_PaymentSchemePlanTest extends CRM_MembershipExtras_Job_OfflineAutoRenewal_BaseRenewalJobTestHelper {
  ...
  public function testRenewalWillRenewPaymentSchemeForExpectedContact() {
    ...
    foreach ($contacts as $contactId) {
      ...
      $paymentPlan = PaymentPlanOrderFabricator::fabricate($paymentPlanMembershipOrder);
      $paymentPlans[$contactId] = $paymentPlan['id'];
    }

    $paymentSchemeRenewal = new PaymentSchemePlanRenewalJob([$firstContact]);
    $paymentSchemeRenewal->run();

    $this->assertTrue($this->isPaymentPlanMembershipRenewed($paymentPlans[$firstContact], '-1 month -1 day', 2));
    $this->assertFalse($this->isPaymentPlanMembershipRenewed($paymentPlans[$secondContact], '-1 month -1 day', 2));
  }
