...
class CRM_MembershipExtras_Service_MembershipInstalmentsSchedule {
  ...
  public function generate(DateTime $startDate = NULL, DateTime $endDate = NULL, DateTime $joinDate = NULL) {
    ...
    $instalments['tax_amount'] = $this->getInstalmentsTotalTaxAmount($instalments['instalments']);
  ...
  /**
   * Calculates Instalments Total Tax Amount
   *
   * @param array $instalments
   * @return float
   */
  private function getInstalmentsTotalTaxAmount(array $instalments) {
    $totalAmount = 0.0;
    foreach ($instalments as $instalment) {
      $totalAmount += $instalment->getInstalmentAmount()->getTaxAmount();
    }
    return $totalAmount;
  }
